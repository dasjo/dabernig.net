 ...
 *   entirety and paste it here, changing the prefix from theme_ to dabernig_design_.
 *   For example:
 *
 *     original: theme_breadcrumb()
 *     theme override: dabernig_design_breadcrumb()
 *
 *   where dabernig_design is the name of your sub-theme. For example, the
...
/* -- Delete this line if you want to use and modify this code
 ...
 if (theme_get_setting('dabernig_design_fixed')) {
 ...
 */
function dabernig_design_theme(&$existing, $type, $theme, $path) {
...
function dabernig_design_preprocess_page(&$vars, $hook) {
  ...
  $vars['css_alt'] = dabernig_design_css_reorder($vars['css']);
  ...
  if (theme_get_setting('dabernig_design_debug')) {
 ...
 */
function dabernig_design_css_reorder($css) {
  global $theme_info;
...
/* -- Delete this line if you want to use this function
 function dabernig_design_preprocess(&$vars, $hook) {
 ...
 */
/* -- Delete this line if you want to use this function
 function dabernig_design_preprocess_node(&$vars, $hook) {
 $vars['sample_variable'] = t('Lorem ipsum.');
 }
 // */
 ...
 */
/* -- Delete this line if you want to use this function
 function dabernig_design_preprocess_comment(&$vars, $hook) {
 $vars['sample_variable'] = t('Lorem ipsum.');
 }
 // */
 ...
 */
/* -- Delete this line if you want to use this function
 function dabernig_design_preprocess_block(&$vars, $hook) {
 $vars['sample_variable'] = t('Lorem ipsum.');
 }
 // */


function dabernig_work_preprocess_views_view_fields(&$vars) {
  if ($vars['view']->name == 'bibliografie') {
    $fields = $vars['fields'];
    if (empty($fields['body']->content) && empty($fields['field_file_fid']->content)) {
      $fields['title']->content = check_plain($fields['title']->raw);
    }
    unset($vars['fields']['body']); 
    unset($vars['fields']['field_file_fid']);
  }
}

function mypagepreprocess (&$vars) {
  if (isset($vars['node']) && ($node = $vars['node'])) {
  
    // Custom breadcrumbs for work and text nodes.
    if ($node->type == 'work' || $node->type == 'text') {
      if (count($node->field_work_category)) {
        $cat = $node->field_work_category[0]['value'];
        $breadcrumb = l($cat, strtolower($cat));
      }
  ...
  if ($vars['title'] == 'Bibliografie' || $vars['title'] == 'Biografie' || $vars['title'] == 'Kontakt' || (isset($node) && ($node->type == 'text' || $node->type == 'work'))) {
    $vars['content_indent'] = true;
  }

  if($vars['title'] == 'Kontakt') {

  }
}

function dabernig_work_preprocess_node(&$node) {
  //panorama page width fix
  if ($node['type'] == 'work' && count($node['field_work_category']) && $node['field_work_category'][0]['value'] == 'Foto') {
    $node['node_width_min'] = 285 * count($node['field_images']) + 20;
  }
  if ($node['nid'] == 335) {
  	$node['node_width_min'] = 4050;
  }
}
